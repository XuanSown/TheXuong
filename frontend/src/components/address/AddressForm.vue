<template>
  <form @submit.prevent="onSubmit" class="flex flex-col gap-4">
    ...
    <div class="flex flex-col gap-1">
      ...
      <input v-model="form.streetDetail" placeholder="Nhập số nhà, tên đường" class="w-full h-[50px] border border-[#CFC4C5] rounded-lg px-3 font-gelasio text-[16px] outline-none focus:border-black" />
      <button type="button" @click="notifyDev" class="self-start text-xs flex items-center gap-1 text-gray-400 underline mt-1">
        Dùng vị trí của tôi
      </button>
      <p class="text-[11px] text-gray-400 italic">Gợi ý địa chỉ &amp; định vị tự động đang được phát triển</p>
    </div>
