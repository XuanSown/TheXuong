<template>
  <header class="fixed top-4 left-1/2 transform -translate-x-1/2 z-50 w-[1278px] max-w-[95vw]">
    ...
    >
      ...
      <div v-if="isAuthenticated" class="flex items-center gap-[16px] absolute right-[24px] top-1/2 transform -translate-y-1/2">
        <!-- Favorite Link -->
        <router-link
          to="/favorite"
          ...
          aria-label="Favorite"
        >
          <svg class="w-[20px] h-[20px] text-[#5E5F5C]" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
      ...
      <div v-else class="flex items-center gap-[16px] absolute right-[24px] top-1/2 transform -translate-y-1/2">
        <router-link
          to="/login"
