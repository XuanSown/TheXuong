        ...
        <div class="grid grid-cols-2 gap-8">
          ...
          <div class="bg-white border border-[#EEEEEE] shadow-[0px_8px_30px_rgba(0,0,0,0.04)] rounded-xl p-8">
            ...
            <form @submit.prevent="onSubmit">
              ...
              <div class="mb-6">
                ...
                <BaseInput v-model="fullName" type="text" placeholder="Nhập họ tên" :error="fullNameError"
                  class="!h-[50px]" />
              </div>
              ...
              <div class="mb-6">
                ...
                <BaseInput v-model="phoneNumber" type="tel" placeholder="Nhập số điện thoại" :error="phoneNumberError"
                  class="!h-[50px]" />
              </div>
              ...
              <div class="flex flex-col gap-2 mb-6">
                ...
                <div class="flex gap-4">
                  <label class="flex items-center gap-2 cursor-pointer">
                    <input v-model="paymentMethod" type="radio" value="COD" class="accent-black" />
                    <span class="font-gelasio text-[14px] text-[#4C4546]">Thanh toán khi nhận hàng (COD)</span>
                  </label>
                  <label class="flex items-center gap-2 cursor-pointer">
                    <input v-model="paymentMethod" type="radio" value="VNPAY" class="accent-black" />
                    <span class="font-gelasio text-[14px] text-[#4C4546]">Chuyển khoản (VNPay)</span>
                  </label>
                </div>
                <span v-if="paymentMethodError" class="text-red-500 text-sm mt-1">{{ paymentMethodError }}</span>
              </div>
              ...
              <div class="flex flex-col gap-2">
                ...
                <textarea v-model="note" rows="2" placeholder="Ghi chú cho đơn hàng..."
                  class="w-full h-[60px] bg-white border border-[#CFC4C5] rounded-lg px-4 py-3 font-gelasio text-[16px] text-[#1A1C1C] outline-none focus:border-black transition-colors resize-none overflow-y-auto"></textarea>
              </div>
            </form>
          </div>
          ...
          <div class="bg-white border border-[#EEEEEE] shadow-[0px_8px_30px_rgba(0,0,0,0.04)] rounded-xl p-8 h-fit">
            ...
            <div v-if="cartItems.length > 0" class="space-y-4 mb-6">
              <div v-for="item in cartItems" :key="`${item.id}-${item.variantId}-${item.size}`"
                class="flex gap-4 pb-4 border-b border-[#E8E8E8]">
                <img :src="item.productImage || '/placeholder.jpg'" alt="" class="w-20 h-20 object-cover rounded-lg"
                  loading="lazy" />
                <div class="flex-1">
            ...
            <!-- Voucher Section -->
            <div v-if="cartItems.length > 0" class="mt-6 mb-2">
              <label
                class="block font-geist text-[12px] font-semibold leading-[12px] tracking-[1.2px] uppercase text-[#4C4546] opacity-80 mb-2">
                MÃ GIẢM GIÁ
              </label>
              <div class="flex gap-2">
                ...
                <BaseButton v-if="!appliedVoucher" @click="applyVoucher" :disabled="!voucherCode || isApplyingVoucher"
                  :loading="isApplyingVoucher" class="!w-auto !px-6">
                  ÁP DỤNG
                </BaseButton>
                <BaseButton v-else @click="removeVoucher" class="!w-auto !px-6 !bg-red-500 hover:!bg-red-600">
                  XÓA
                </BaseButton>
              </div>
              <p v-if="voucherMessage" :class="voucherError ? 'text-red-500' : 'text-green-500'"
                class="text-sm mt-2 font-gelasio">
                {{ voucherMessage }}
              </p>

              <!-- My Vouchers List -->
              <div v-if="myVouchers.length > 0 && !appliedVoucher" class="mt-4">
                <button type="button" @click="showVoucherList = !showVoucherList"
                  class="font-gelasio text-[14px] text-blue-600 hover:underline font-semibold flex items-center gap-1">
                  <svg class="w-4 h-4 transition-transform" :class="{ 'rotate-180': showVoucherList }" fill="none"
                ...
                <div v-if="showVoucherList"
                  class="flex flex-col gap-2 mt-3 bg-gray-50 p-4 rounded-lg border border-[#EEEEEE]">
                  <p class="text-xs text-gray-500 mb-1">Click vào mã để tự động điền và áp dụng</p>
                  <div class="flex flex-wrap gap-2">
                    <button v-for="uv in myVouchers" :key="uv.id" type="button" @click="applyMyVoucher(uv.code)"
            ...
            <!-- Points Section -->
            <div v-if="currentPoints > 0" class="mt-6 mb-2">
              <label
                class="block font-geist text-[12px] font-semibold leading-[12px] tracking-[1.2px] uppercase text-[#4C4546] opacity-80 mb-2">
                SỬ DỤNG ĐIỂM THƯỞNG
              </label>
              <p class="text-sm font-gelasio text-[#4C4546] mb-2">Bạn đang có <strong>{{ formatPrice(currentPoints) }}</strong> (1 điểm = 1đ)</p>
              <div class="flex gap-2">
                <input v-model.number="pointsToUse" type="number" min="0" :max="currentPoints" placeholder="Nhập số điểm..."
                  class="flex-1 h-[48px] px-4 border border-[#EEEEEE] rounded-lg focus:outline-none focus:border-black font-gelasio" />
                <BaseButton @click="applyAllPoints" class="!w-auto !px-4 !bg-[#f1c40f] hover:!bg-[#f39c12] !text-black">
                  DÙNG TẤT CẢ
                </BaseButton>
              </div>
              <p v-if="pointsError" class="text-red-500 text-sm mt-2 font-gelasio">
                {{ pointsError }}
              </p>
            </div>
            ...
            <div v-if="cartItems.length > 0" class="space-y-3 pt-4 border-t border-[#E8E8E8]">
              ...
              <div v-if="tierDiscountAmount > 0" class="flex justify-between text-green-600">
              ...
              <div v-if="pointsToUse > 0" class="flex justify-between text-green-600">
            ...
            <!-- Place Order Button -->
            <BaseButton v-if="cartItems.length > 0" @click="onSubmit" :disabled="cartItems.length === 0"
              :loading="isSubmitting" class="w-full mt-6 !h-[56px]">
              ĐẶT HÀNG
            </BaseButton>
          </div>
        </div>
...
const loadCheckoutData = async () => {
  ...
  try {
    ...
    if (response.data) {
      autoDiscountPercent.value = response.data.autoDiscountPercent || 0
      tierDiscountAmount.value = response.data.tierDiscountAmount || 0
      currentPoints.value = response.data.currentPoints || 0
      ...
      if (response.data.availableVouchers) {
        myVouchers.value = response.data.availableVouchers
      }
    }
  } catch (error) {
...
const applyVoucher = async () => {
  ...
  try {
    const response = await api.get('/loyalty/validate-voucher', {
      params: {
...
const onSubmit = handleSubmit(async (values) => {
  ...
  try {
    ...
    if (response.data.order.paymentUrl) {
      window.location.href = response.data.order.paymentUrl
    } else {
