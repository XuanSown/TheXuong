      ...
      <div class="w-[1152px] mx-auto">
        ...
        <div class="flex gap-8 border-b border-[#CFC4C5] mb-6">
          <button v-for="tab in tabs" :key="tab.value" @click="activeTab = tab.value"
            ...
            {{ tab.label }}
            <div v-if="activeTab === tab.value" class="absolute bottom-[-1px] left-0 w-full h-[2px] bg-black"></div>
          </button>
        </div>
        ...
        <div
          ...
          <div class="flex border-b border-[#CFC4C5] bg-[#F3F3F3]">
            ...
            <div class="w-[183.5px] h-[44px] flex items-center justify-end px-8">
              <span
                class="font-geist text-[12px] font-semibold leading-[12px] tracking-[1.8px] text-[#5E5F5C] text-right">CHI
                TIẾT</span>
            </div>
          </div>

          <!-- Loading State -->
          <div v-if="orderStore.loading" class="flex items-center justify-center py-16">
            <div class="w-8 h-8 border-4 border-black border-t-transparent rounded-full animate-spin"></div>
          </div>

          <!-- Error State -->
          <div v-else-if="orderStore.error" class="flex flex-col items-center justify-center py-16">
            <p class="font-gelasio text-lg text-red-500 mb-4">{{ orderStore.error }}</p>
          ...
          <div v-else-if="orders.length === 0" class="flex flex-col items-center justify-center py-16">
            <p class="font-gelasio text-lg text-[#5E5F5C] mb-4">Bạn chưa có đơn hàng nào</p>
            <router-link to="/products" class="px-6 py-2 bg-black text-white text-sm rounded hover:bg-gray-900">
              Mua sắm ngay
            </router-link>
          </div>
          ...
          <div v-else class="max-h-[400px] overflow-y-auto">
            <div v-for="order in filteredOrders" :key="order.id"
              ...
              <div class="w-[317.3px] h-[110px] flex flex-col justify-center px-6">
                ...
                <span class="font-geist text-[14px] text-[#5E5F5C] truncate" :title="order.address">{{ order.address
                  }}</span>
              </div>
              <!-- Total -->
              ...
              <div class="w-[194.73px] h-[110px] flex items-center px-6">
                <span :class="['inline-flex items-center gap-2 px-3 py-1 rounded-full font-geist text-[12px] uppercase tracking-[0.6px]',
